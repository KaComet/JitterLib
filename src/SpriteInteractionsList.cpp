...
bool SpriteInteractionsList::loadSpriteInteractions(const std::string &fileName, const std::string &tag) {
    ...
    if (inputFile.is_open()) {
        ...
        while (!inputFile.eof()) {
            ...
            if (isLoadingEnabled) {
                ...
                std::optional<walt::UI::SpriteInteraction> resultOpt = loadSI(currentLine);

                // If the SI exists, add it to storage.
                if (resultOpt.has_value()) {
                    walt::UI::SpriteInteraction &result = resultOpt.value();
...
std::optional<walt::UI::SpriteInteraction> SpriteInteractionsList::getSI(const Material &material) {
    std::optional<walt::UI::SpriteInteraction> result;
...
std::optional<FrameDef> SpriteInteractionsList::get(const flat::IntegerCoordinate &cord, walt::Display::DisplayData &dis,
                                                    FrameDefMap &frameDefs, bool getBackground) {
    ...
    std::optional<walt::UI::SpriteInteraction> thisInteraction = getSI(thisMaterial);
...
std::optional<FrameDef> SpriteInteractionsList::getFrameDef(const std::string &name, FrameDefMap &frameDefs) {
    walt::UI::SpriteInteraction resultSI;
    try {
...
FrameDef SpriteInteractionsList::getInteraction(const Material &material,
...
std::optional<walt::UI::SpriteInteraction> SpriteInteractionsList::getFullSpriteInteraction(const std::string &input) {
    walt::UI::SpriteInteraction result;
...
walt::UI::SpriteInteraction SpriteInteractionsList::getSimpleSpriteInteraction(const std::string &input) {
    std::string relevant = getEncapsulatedContents(input, '[', ']', 1);
    relevant = getEncapsulatedContents(relevant, '[', ']', 1);
...
std::string SpriteInteractionsList::getSI_Key(const std::string &input) {
    std::string tmp = getEncapsulatedContents(input, '[', ']', 0);
    return getEncapsulatedContents(tmp, '<', '>', 0);
}

bool SpriteInteractionsList::isSimpleSI(const std::string &input) {
...
bool SpriteInteractionsList::isFullyDefined(const walt::UI::SpriteInteraction &SI) {
...
bool SpriteInteractionsList::isMultiLine(const std::string &input) {
    if (input.size() < 3)
        return false;
...
std::optional<walt::UI::SpriteInteraction> SpriteInteractionsList::loadSI(const std::string &input) {
    std::optional<walt::UI::SpriteInteraction> result;
    ...
    return std::optional<walt::UI::SpriteInteraction>(result);
}
