...
bool SpriteSet::loadFromFile(SDL_Renderer *renderer, const std::string &path, uint tileWidth, uint tileHeight) {
...
bool SpriteSet::render(unsigned int x, unsigned int y, walt::UI::FrameID valueToDisplay) const {
...
bool SpriteSet::render(unsigned int x, unsigned int y, walt::UI::FrameID frameToDisplay, SDL_Color renderColor) {
    if (!mBitmap)
        return false;
...
void SpriteSet::setRenderer(SDL_Renderer *renderer) {
    mBitmap->setRenderer(renderer);
}
